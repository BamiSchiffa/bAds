...
module.exports = {
	...
	execute: async (client, message) => {
		...
		message.guild.config = client.config.prefix;
		...
		if (command.permission) {
			if ((command.permission == `BECO_OWNER` && !client.config.owners.includes(message.author.id)) || !message.member.hasPermission(command.permission)) return message.channel.send(client.embed.small.error(`You don't have permission to perform this command. (\`${command.permission}\`)`));
		}
